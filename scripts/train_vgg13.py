...
def train_fold(train_paths, val_paths, args, device, fold):
    ...
    train_loader = DataLoader(PaperDataset(train_paths, train=True), batch_size=args.batch_size, shuffle=True, num_workers=0)
    val_loader = DataLoader(PaperDataset(val_paths, train=False), batch_size=args.batch_size, shuffle=False, num_workers=0)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch == 1 or epoch % log_every == 0 or epoch == args.epochs:
            print(f"  fold {fold} epoch {epoch:3d}/{args.epochs} | loss={train_loss:.4f} | "
                  f"val acc={metrics.accuracy:.3f} f1={metrics.f1:.3f} auc={metrics.auc:.3f} | "
                  f"best_f1={best_f1:.3f} | {time.time()-t0:.1f}s", flush=True)
...
def main() -> None:
    ...
    print(f"Config: folds={args.folds} epochs={args.epochs} bs={args.batch_size} lr={args.lr} thickness={args.thickness} seed={args.seed}")
    ...
    for fold_idx, (train_paths, val_paths) in enumerate(folds_to_run):
        print(f"\n=== Fold {fold_idx+1}/{args.folds} | train={len(train_paths)} val={len(val_paths)} ===", flush=True)
        metrics, (y_true, y_prob), _ = train_fold(train_paths, val_paths, args, device, fold_idx)
        per_fold.append(metrics)
        save_predictions(PRED_CSV, val_paths, y_true, y_prob, fold=fold_idx, seed=args.seed, model=args.tag)
        print(f"  -> fold {fold_idx}: acc={metrics.accuracy:.3f} f1={metrics.f1:.3f} auc={metrics.auc:.3f}", flush=True)
