...
from src.metrics import aggregate_folds, save_predictions
...
def main() -> None:
    ...
    print(f"Device: {device} | Tag: {args.tag} | transform: {'imagenet' if args.no_equalize else 'paper'}")
    print(f"Config: folds={args.folds} epochs={args.epochs} bs={args.batch_size} lr={args.lr} wd={args.weight_decay} seed={args.seed}")
    ...
    for fold_idx, (train_paths, val_paths) in enumerate(folds_to_run):
        print(f"\n=== Fold {fold_idx+1}/{args.folds} | train={len(train_paths)} val={len(val_paths)} ===")
        ...
        metrics, y_true, y_prob, _ = train_one_fold(build_resnet50, train_ds, val_ds, cfg, fold=fold_idx)
