...
COLORS = {"resnet50": "#1f77b4", "dinov2": "#2ca02c", "clip": "#9467bd", "ensemble": "#ff7f0e"}
LABELS = {"resnet50": "ResNet50", "dinov2": "DINOv2", "clip": "CLIP", "ensemble": "Ensemble"}
...
def load_backbone(backbone: str, paths: list[Path]) -> tuple[np.ndarray, np.ndarray]:
    if backbone == "ensemble":
        ...
        for b in ["resnet50", "dinov2", "clip"]:
            ...
            parts.append(data["X"])
        return np.concatenate(parts, axis=1), data["y"]
    data = np.load(CACHE_DIR / f"embeddings_{backbone}.npz", allow_pickle=True)
    return data["X"], data["y"]


def subsample_stratified(idx: np.ndarray, y: np.ndarray, frac: float, rng: np.random.Generator) -> np.ndarray:
    keep = []
...
def main() -> None:
    ...
    for backbone in BACKBONES:
        ...
        for frac in FRACTIONS:
            ...
            for fold_idx, (train_paths, val_paths) in enumerate(folds):
                ...
                clf = LogisticRegression(C=1.0, max_iter=4000, solver="lbfgs", random_state=42 + fold_idx)
                clf.fit(scaler.transform(X_tr), y_tr)
                fold_accs.append((clf.predict(scaler.transform(X_va)) == y_va).mean())
            mean, std = float(np.mean(fold_accs)), float(np.std(fold_accs))
            means.append(mean); stds.append(std); ns.append(n_train_for_log)
            print(f"  {backbone:10s} frac={frac:.2f} n_train={n_train_for_log:3d} acc={mean:.3f} ± {std:.3f}")
        curves[backbone] = {"n": ns, "mean": means, "std": stds}

    fig, ax = plt.subplots(figsize=(7, 5))
    for backbone, c in curves.items():
        n = np.array(c["n"]); m = np.array(c["mean"]); s = np.array(c["std"])
    ...
    ax.axhline(0.65, color="#d62728", linestyle="--", linewidth=0.8, label="VGG13 from scratch (215 imgs)")
    ax.axhline(0.89, color="red", linestyle=":", linewidth=0.8, label="paper claim (215 imgs)")
    ...
    (RESULTS_DIR / "tables" / "data_efficiency.json").write_text(json.dumps(curves, indent=2))
