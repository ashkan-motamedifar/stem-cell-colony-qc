...
class TrainConfig:
    ...
    device: str = "mps"
    seed: int = 42
    num_workers: int = 0  # MPS + multi-worker is buggy
    log_every: int = 10
...
@torch.no_grad()
def evaluate(model, loader, device) -> tuple[BinaryMetrics, list[int], list[float]]:
    model.eval()
    y_true, y_prob = [], []
    for x, y in loader:
...
def train_one_fold(
    ...
    best_acc, best_state, best_preds, best_metrics = 0.0, None, (None, None), None
    t0 = time.time()
    for epoch in range(1, cfg.epochs + 1):
        ...
        if epoch == 1 or epoch % cfg.log_every == 0 or epoch == cfg.epochs:
            print(f"  fold {fold} epoch {epoch:3d}/{cfg.epochs} | train_loss={loss:.4f} | "
                  ...
                  f"best_acc={best_acc:.3f} | elapsed={time.time()-t0:.1f}s")
